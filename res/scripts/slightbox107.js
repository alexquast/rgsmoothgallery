...
gallery = gallery.extend({
    ...
    makeLink: function(num) {
        ...
        if (options.lightbox)
            this.currentLink.onclick = this.lightbox2.open.pass(num, this.lightbox2);
...
var lightboxforsmoothgallery = LightboxSmoothgallery.extend({
    initialize: function(galleryData, options) {
        ...
        if (buttons.length > 0) {
            ...
            if (buttonsConf == 10 || buttonsConf == 11)
                new Element('a').setProperties({id: 'rglbSaveLink', href: 'javascript:void(0)'}).injectInside(this.bottom).onclick = this.printOrSave.bind(this, 'save');
            if (buttonsConf == 01 || buttonsConf == 11)
    ...
    changeImage: function(num) {
        if (this.step || (num < 0) || (num >= this.images.length))
            return false;
        this.step = 1;
    ...
    nextEffect: function() {
        switch (this.step++) {
            case 1:
                ...
                if (this.activeImage != 0)
                    this.preloadPrev.src = this.images[this.activeImage - 1].imagepath;
                if (this.activeImage != (this.images.length - 1))
                    this.preloadNext.src = this.images[this.activeImage + 1].imagepath;
                if (this.center.clientHeight != this.image.offsetHeight + 20) {
                    this.fx.resize.custom({height: [this.center.clientHeight, this.image.offsetHeight]});
                    break;
                }
