...
var HistoryManager = {
    ...
    setValues: function(key, values) {
        ...
        if (data.values.isSimilar(values))
            return this;
        data.values = values;
        this.update();
        return this;
    ...
    observe: function() {
        ...
        if (this.state == state)
            return;
        if (window.ie || window.oldsafari && (this.state != null))
    ...
    generateState: function() {
        var state = [];
        this.modules.each(function(data, key) {
            if (data.values.isSimilar(data.defaults) && data.skipDefaultMatch)
                return;
    ...
    setState: function(state, fix) {
        ...
        if (window.oldsafari) {
            if (!this.form)
                this.form = new Element('form', {'method': 'get'}).injectInside(document.body);
            this.count = history.length;
...
if (window.khtml && navigator.userAgent.test('AppleWebKit')) {
    ...
    if (ver && parseInt(ver[1]) < 420)
        window.oldsafari = true;
}