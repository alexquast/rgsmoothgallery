...
Element.extend({
    ...
    removeClass: function(className) {
        if (this.hasClass(className))
            this.className = this.className.replace(className, '').clean();
        return this;
    },
    /*
    ...
    setStyle: function(property, value) {
        if (property == 'opacity')
            ...
            this.style[property.camelCase()] = (value.push) ? value.rgbToHex() : value;
        return this;
    },
    /*
     Property: setStyles
        ...
        switch ($type(source)) {
            case 'object':
                for (var property in source)
                    this.setStyle(property, source[property]);
                break;
            case 'string':
                if (window.ie)
                    this.cssText = source;
                else
                    this.setAttribute('style', source);
        }
        return this;
    },
    /*
    ...
    getStyle: function(property) {
        ...
        if (!$chk(style)) {
            ...
            if (document.defaultView)
                style = document.defaultView.getComputedStyle(this, null).getPropertyValue(property.hyphenate());
    ...
    addEvents: function(source) {
        if (source) {
            for (var type in source)
                this.addEvent(type, source[type]);
        }
        return this;
    },
    /*
                ...
                if (window.ie6) {
                    ...
                    $each(this.attributes, function(attribute) {
                        if (attribute.name != 'name')
                            el.setProperty(attribute.name, attribute.value);
                    });
                    if (this.parentNode)
                        this.replaceWith(el);
                    return el;
    ...
    setProperties: function(source) {
        for (var property in source)
            this.setProperty(property, source[property]);
        return this;
    },
    /*
     Property: setHTML
    ...
    setOptions: function(defaults, options) {
        ...
        if (this.addEvent) {
            for (var option in this.options) {
                if (($type(this.options[option]) == 'function') && option.test('^on[A-Z]'))
        ...
        selector.clean().split(' ').each(function(sel, i) {
            ...
            if (i == 0) {
                if (param[2]) {
                    ...
                    if (!el || ((param[1] != '*') && (Element.prototype.getTag.call(el) != param[1])))
                        return;
                    filters = [el];
                } else {
                ...
                if (param[2])
                    filters = Elements.prototype.filterById.call(filters, param[2]);
            }
            if (param[3])
                filters = Elements.prototype.filterByClassName.call(filters, param[3]);
            if (param[4])
                filters = Elements.prototype.filterByAttribute.call(filters, param[4], param[6], param[5]);
        }, this);
        return $$(filters);
    },
    /*
...
Elements.extend({
    ...
    filterByClassName: function(className) {
        var found = [];
        this.each(function(el) {
            if (Element.prototype.hasClass.call(el, className))
                found.push(el);
        });
        return found;
    },
...
var Hash = new Class({
    ...
    remove: function(key) {
        ...
        for (var property in this.obj) {
            if (property != key)
                obj[property] = this.obj[property];
        }
        this.obj = obj;
        return this;
    },
    each: function(fn, bind) {
        for (var property in this.obj)
            fn.call(bind || this, property, this.obj[property]);
    },
    extend: function(obj) {
...
var Color = new Class({
    ...
    mix: function() {
        ...
        var alpha = 50;
        if ($type(colors[colors.length - 1]) == 'number')
            alpha = colors.pop();
        var rgb = this.copy();
...
Fx.Scroll = Fx.Base.extend({
    ...
    scrollTo: function(x, y) {
        ...
        for (var z in el.size) {
            ...
            if ($chk(values[z]))
                values[z] = ($type(values[z]) == 'number') ? Math.max(Math.min(values[z], max), 0) : max;
            else
